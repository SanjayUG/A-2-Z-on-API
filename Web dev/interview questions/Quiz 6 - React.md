...
1.  How would you prevent setting state inside `useEffect` after a component has unmounted to avoid memory leaks in asynchronous operations?
    ...
    **Answer:** Use a cleanup function with a mounted flag or `AbortController` to cancel async calls
...
2.  What arguments does `useMemo` take?
    ...
    **Answer:** A callback function and a dependency array
...
3.  What are potential pitfalls when overusing `useMemo` in React components?
    ...
    **Answer:** Unnecessary complexity and memory overhead without noticeable performance gains
...
4.  When does `useMemo` recompute its memoized value?
    ...
    **Answer:** Only when one of the dependencies changes
...
5.  What is the purpose of the `useMemo` hook?
    ...
    **Answer:** To memoize the result of a calculation to optimize performance
...
7.  What issues might arise if you omit dependency array or specify dependencies incorrectly in `useEffect`?
    ...
    **Answer:** Infinite loops or stale closures causing bugs
...
8.  If a `useEffect` has a cleanup function, when is this cleanup function executed?
    ...
    **Answer:** Before the next effect runs or when the component unmounts
...
9.  Which of the following is a common use case of `useEffect`?
    ...
    **Answer:** Fetching data from an API after the component mounts
...
10. What is the main purpose of `forwardRef` in React?
    ...
    **Answer:** To forward a ref through a component to a child component or DOM element
...
11. What could happen if you omit the dependency array from `useMemo`?
    ...
    **Answer:** The memoized value is recomputed on every render, losing optimization benefits
...
13. Which of the following is a benefit of using `useMemo`?
    ...
    **Answer:** Prevents expensive computations on every render
...
14. How is `useRef` commonly used in React components?
    ...
    **Answer:** To store a reference to a DOM element or a mutable value that does not cause re-render on change
...
15. How can you access a DOM node of a child functional component using refs?
    ...
    **Answer:** Wrap the child component with `forwardRef` and pass the ref from the parent
...
16. Which of the following best describes the primary purpose of a custom hook in React?
    ...
    **Answer:** To encapsulate reusable logic and stateful behavior that can be shared across components
...
17. How often will the effect run if you pass an empty dependency array `[]` to `useEffect`?
    ...
    **Answer:** Once after the initial render only
...
18. What happens if you include a state variable in the dependency array of `useEffect`?
    ...
    **Answer:** The effect runs after the initial render and whenever that state changes
...
19. What is the purpose of the dependency array as the second argument in `useEffect`?
    ...
    **Answer:** To specify when the effect should rerun based on changes in dependencies
...
20. What problem does `forwardRef` solve in React component composition?
    ...
    **Answer:** It allows parent components to directly access child component's DOM nodes or instance methods, which normally isn’t possible with function components
...
21. How do you define a React component that supports forwarding refs?
    ...
    **Answer:** Wrap it with `React.forwardRef` and accept `ref` as the second argument
...
22. What kind of function can you optionally return from a `useEffect` callback?
    ...
    **Answer:** A cleanup function that runs before the next effect or when the component unmounts
...
23. What does the `useEffect` hook primarily allow you to do in React function components?
    ...
    **Answer:** Perform side effects like data fetching or DOM manipulation
...
24. In which scenario is `useMemo` particularly helpful?
    ...
    **Answer:** When you have a costly computation that depends on certain inputs and should be recalculated only when those inputs change
...
25. What is a key rule you must follow when creating a custom hook in React?
    ...
    **Answer:** Its name must start with `use` to allow React to recognize it as a hook